...
app.post("/add_group", async (request, response) => {
    let newTeamModel = request.body,
        responseObj = {},
        teams = await teamsModel.find({}),
        result;

    console.log(newTeamModel);

    try {
        if(teams.filter((item) => item.team === newTeamModel.team).length >0) {
            result = await teamsModel.updateOne(
                {team: newTeamModel.team},
                { $set: { employees: newTeamModel.employees}}
            );
...
app.get("/review", async (request, response) => {
    const query = request.query.provider,
    queryObj = (query && query.length>0) ? {provider: query} : '';

    try {
        let review = queryObj ? await empReviewModel.find(queryObj) : await empReviewModel.find(),
            responseObj = {};
...
app.post("/login", express.json({type: '*/*'}), async (request, response) => {
    ...
    try {
        const employees = await empModel.find({}),
            loginEmp = employees.filter(emp => emp.id === user.id && emp.password === user.password);
        ...
        // response.status(responseObj.code).json(JSON.stringify(responseObj));
...
app.post("/update_employee", async (request, response) => {
    console.log(request.body);
    const user = request.body,
        result = await empModel.updateOne(
            {id: user.id},
            { $set: { name: user.name, role: user.role, team: user.team}}
        );
    let responseObj = {};
    console.log(result);
    try {
...
app.post("/delete_employee", async (request, response) => {
    ...
    let responseObj = {};
    const user = request.body,
        result = await empModel.deleteOne(
            {id: user.id},
            (err, obj) => {
                if (err) {
                    throw err;
