...
class PerformanceDashboard extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            rating: '', comment: '', empListForFeedback: [], empListOfSentFeedback: [], showFeedbackModal: false, selectedEmpName: '', selectedEmpId: ''
        };
    ...
    modalFeedbackClose() {
        this.setState({
            comment: '', rating: '', showFeedbackModal: false
        });
    }

    submitFeedback = () => {
        // submit performance dashboard feedback
        if (this.state.comment.length > 0 && this.state.rating !== '') {
            const url = common.urls.add_review, data = JSON.stringify({
                name: this.state.selectedEmpName,
                id: this.state.selectedEmpId,
                provider: this.props.LoggedInUser.name + '-' + this.props.LoggedInUser.id,
                feedback: {comment: this.state.comment, rating: this.state.rating}
            });
            axios.post(url, data)
                .then(response => {
    ...
    getTeamList() {
        // fetching team list to show in table, for performace review
        ...
        axios.get(url)
            .then(response => {
                if (response.data.status === 'success') {
                    ...
                    Toast.fail(response.data.message + ', please contact admin to proceed', 2500);
                }
            })
            .catch(error => {
    ...
    componentDidMount() {
        // get list of employees who's performace review is not/not, and highlight based on this list
    ...
    render() {
        ...
        const renderEmpList = empListForFeedback
            .map((item, index) => {
                ...
                return (<tr className={isFeedbackSend ? 'disable-notification' : ''} key={'message-' + index}>
                    {<td>
                        <i className={isFeedbackSend ? 'disable-notification fa fa-bell ' : 'alert-notification fa fa-bell '} title={isFeedbackSend ? 'feedback send' : 'feedback pending'}></i>
                    </td>}
                    <td>{item.name}</td>
                    <td>{item.id}</td>
                    <td>{LoggedInUser.team}</td>
                    <td><i className="fa fa-paper-plane fa-2x icon-button" title='send feedback' aria-hidden="true" onClick={() => this.handleFeedbackModal(item,
                        isFeedbackSend)}></i></td>
                </tr>)
            });
