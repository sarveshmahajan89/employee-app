...
class Login extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            userId: '', password: '', isUserLoggedIn: false
        };
    ...
    submitLogin = e => {
        // submitting user credentials for validating, and login to portal
        e.preventDefault();
        ...
        axios.post(url, JSON.stringify({id: this.state.userId, password: this.state.password}))
            .then(response => {
                if (response.data.status === 'success') {
                    sessionStorage.setItem(common.loginUserDetails, JSON.stringify(response.data.user));
    ...
    render() {
        ...
        if (isUserLoggedIn) {
            // after login default route is home screen
            return <Redirect to="/"/>
        }
