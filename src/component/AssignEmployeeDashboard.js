...
class AssignEmployeeDashboard extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            checkBoxObj: {}, team: '', selectedEmp: [], showConfirmModal: false
        };
    ...
    empSelection = (id) => {
        // set selected employee for review grouping
        this.setState({
            checkBoxObj: {
    ...
    handleTeamChange = e => {
        // getting list of employees for respective teams before grouping
    ...
    addGroup = (e) => {
        // adding employees to group for performance review
        if (this.state.team && this.state.team !== '') {
            let selectedTeam = this.props.employeeList.filter((item) => item.team === this.state.team), empGroupList = [];
            selectedTeam.map(item => {
                empGroupList.push({id: item.id, name: item.name});
            });
            const url = common.urls.add_group, data = JSON.stringify({team: this.state.team, employees: empGroupList, active: true});
            axios.post(url, data)
                .then(response => {
                    if (response.data.status === 'success') {
    ...
    render() {
        ...
        let teamList = employeeList.length > 0 ? (employeeList.map(item => item.team)).filter((v, i, a) => {
                    ...
                    <div className="menu-item-section btn-toolbar" role="toolbar">
                        <div className="menu-item btn-group">
                            <select className="form-control" value={team} name="team" onChange={this.handleTeamChange}>
                                ...
                                {teamList.map((item, index) => (<option key={index} value={item}>{item}</option>))}
                            </select>
                        </div>
                        <div className="menu-item btn-group">
